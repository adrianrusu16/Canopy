...
-- Personal library tracks are seeded from fixtures/media/personal. They are
-- promoted so playback works without a configured instance owner, which the
-- disposable environment never sets.
UPDATE licenses AS license
SET review_status = 'approved',
    reviewed_at = COALESCE(license.reviewed_at, NOW())
FROM tracks AS track
...
WHERE license.id = track.license_id
  AND provider_track.provider = 'fixture'
  AND provider_track.provider_track_id LIKE 'personal-%';
