...
async fn local_environment_auth_and_playback_smoke() {
    ...
    assert!(
        source
            .stream_url
            .starts_with(&format!("{public_stream_endpoint}/stream/")),
        "playback must use the configured public Nginx stream origin"
    );
    ...
    let mut local_stream_url = Url::parse(&endpoint(
        "CANOPY_LOCAL_STREAM_ENDPOINT",
        "http://127.0.0.1:8080",
    ))
    .expect("local stream endpoint should be valid");
    local_stream_url.set_path(stream_url.path());
    local_stream_url.set_query(stream_url.query());
    let response = client
        .get(local_stream_url)
