...
emulator_grpc_endpoint="http://10.0.2.2:50051"
emulator_stream_endpoint="http://10.0.2.2:8080"
emulator_openapi_endpoint="http://10.0.2.2:8080/openapi.json"
...
stop_managed() {
  local stop_status=0
  if [[ -f "$pid_file" ]]; then
    stop_canopy || stop_status=1
  fi
  if [[ -f "$runtime_env" ]]; then
    load_runtime
    compose down --remove-orphans || stop_status=1
  fi
  return "$stop_status"
}
...
case "$1" in
  up)
    ...
    cleanup_on_exit=0
    printf '%s\n' \
      "gRPC: $emulator_grpc_endpoint" \
      "Streaming: $emulator_stream_endpoint" \
      "OpenAPI: $emulator_openapi_endpoint" \
      "Test inbox: $mailpit_endpoint"
    ;;
  test)
    validate_prerequisites
    state_present && die "reset the persistent environment before running the disposable test"
    cleanup_on_exit=1
    start_environment
    cargo test -p canopy-server --features pg --test local_integration \
